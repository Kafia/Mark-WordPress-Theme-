...
class Breadcrumb_Trail {
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => __( 'Browse:',                 'frogsthemes' ),
			'home'                => __( 'Home',                    'frogsthemes' ),
			'homeextra'           => __( '',                        'frogsthemes' ),
			'search'              => __( 'Search results for "%s"', 'frogsthemes' ),
			'error_404'           => __( '404 Not Found',           'frogsthemes' ),
			'paged'               => __( 'Page %d',                 'frogsthemes' ),
			'archives'            => __( 'Archives',                'frogsthemes' ),
			'archive_minute_hour' => __( 'g:i a',                   'frogsthemes' ),
			'archive_minute'      => __( 'Minute %d',               'frogsthemes' ),
			'archive_hour'        => __( 'g a',                     'frogsthemes' ),
			'archive_day'         => __( 'd',                       'frogsthemes' ),
			'archive_week'        => __( 'Week %d',                 'frogsthemes' ),
			'archive_month'       => __( 'F',                       'frogsthemes' ),
			'archive_year'        => __( 'Y',                       'frogsthemes' ),
		//	'edit'                => __( 'Edit',                    'frogsthemes' ), // @todo Implement edit link
		);
	...
	public function do_site_home_link() {
		...
		$this->items[] = '<a href="' . home_url() . '" title="' . esc_attr( get_bloginfo( 'name' ) ) . '">' . $label . '</a>' . $this->args['labels']['homeextra'];
	}
		...
		if ( true === $this->args['show_on_front'] ) {
			...
			/* If on the main front page, add the network home link item and the home item. */
				...
				if ( true === $this->args['show_title'] )
					if($this->args['labels']['homeextra']):
						$this->items[] = ( is_multisite() && true === $this->args['network'] ) ? get_bloginfo( 'name' ) : '<a href="'.home_url().'">'.$this->args['labels']['home'].'</a> '. $this->args['labels']['homeextra'];
					else:
	...
	public function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '" title="' . get_the_time( __( 'Y', 'frogsthemes' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_year'], $post_id ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F Y', 'frogsthemes' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_month'], $post_id ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '" title="' . get_the_time( esc_attr__( 'F j, Y', 'frogsthemes' ), $post_id ) . '">' . get_the_time( $this->args['labels']['archive_day'], $post_id ) . '</a>';
...
class bbPress_Breadcrumb_Trail extends Breadcrumb_Trail {
	...
	public function do_trail_items() {
		...
		elseif ( bbp_is_topic_tag_edit() ) {
			...
			if ( true === $this->args['show_title'] )
				$this->items[] = __( 'Edit', 'frogsthemes' );
		}
